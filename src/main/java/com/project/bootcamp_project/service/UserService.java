...
import com.project.bootcamp_project.specification.UserSpecification;
    ...
    public ResponseEntity<Object> refreshToken(HttpServletRequest request) {
        try {
            ...
            if (refreshToken == null) {
                ...
                return DefaultResponse.invalidRefreshTokenNotPresent(request);
            }
...
//                    new HashMap<>(),
    ...
    public ResponseEntity<Object> searchUser(String email,
        ...
        Specification<User> specification = UserSpecification.getUserSpecification(
                email,
                roleName,
                id,
                createdAtStart,
        ...
        List<UserResponseDTO> res = users.stream()
                .map(user -> new UserResponseDTO(
                        ...
                        user.getEmail(),
                        user.getRole().getName(),
                        user.getRole().getAccessPermissions().stream()
                                .map(AccessPermission::getName)
                                .collect(Collectors.toSet())
                ))
                .toList();
